...
from dict_computation import get_SCOP_node_label_dict


def plot_embedding(X, Y, n, th):
    ...
    # plt.show()
    plt.axis('off')
    plt.savefig(f"outputs/images/remote_homologs_at_th_{th}_n_{n}.png", dpi=300, format="png", bbox_inches='tight', pad_inches=0.0)

for th in  [20]: # seq_identity_th #[20, 25, 35, 40, 95]
    for n in [1, 5, 10, 20, 30]: # at_least_n_proteins_per_sfam [1, 5, 10, 20, 30]
        
        data = Utils.load_pickle(f"data/tmp/remote_homologs/remote_homologs_at_th_{th}_n_{n}.pkl")
        X = np.stack(data[:, 2])
        Y = data[:, 1]
